...
class CalculatingTheHouse: UIViewController {
    enum DataType {
        case finishing
        case engineering
    }
    
    struct FinishingCategory {
        enum Selection {
            case single // одна
    ...
    let finishingData: [FinishingCategory] = [
        FinishingCategory(
            title: "Комплект дома",
            ...
            selectionType: .single,
            options: [FinishingOption(id: "1", title: "Террасная доска", deltaRub: 262559),FinishingOption(
                    id: "1",
    ...
    let engineeringData: [FinishingCategory] = [
        FinishingCategory(
            title: "Отопление",
            selectionType: .single,
            options: [
                FinishingOption(id: "h1", title: "Электрический котел", deltaRub: 85000),
                FinishingOption(id: "h2", title: "Газовый котел", deltaRub: 120000),
                FinishingOption(id: "h3", title: "Твердотопливный котел", deltaRub: 95000),
                FinishingOption(id: "h4", title: "Тепловой насос", deltaRub: 350000),
                FinishingOption(id: "h5", title: "Радиаторы отопления", deltaRub: 89000),
                FinishingOption(id: "h6", title: "Теплый пол водяной", deltaRub: 145000)
            ]
        ),
        FinishingCategory(
            title: "Водоснабжение",
            selectionType: .multiple,
            options: [
                FinishingOption(id: "w1", title: "Скважина", deltaRub: 120000),
                FinishingOption(id: "w2", title: "Насосная станция", deltaRub: 45000),
                FinishingOption(id: "w3", title: "Фильтрация воды", deltaRub: 35000),
                FinishingOption(id: "w4", title: "Водонагреватель", deltaRub: 55000),
                FinishingOption(id: "w5", title: "Трубы ХВС", deltaRub: 67000),
                FinishingOption(id: "w6", title: "Трубы ГВС", deltaRub: 67000),
                FinishingOption(id: "w7", title: "Коллектор водоснабжения", deltaRub: 28000)
            ]
        ),
        FinishingCategory(
            title: "Канализация",
            selectionType: .multiple,
            options: [
                FinishingOption(id: "s1", title: "Септик", deltaRub: 95000),
                FinishingOption(id: "s2", title: "Трубы канализации", deltaRub: 45000),
                FinishingOption(id: "s3", title: "Ливневая канализация", deltaRub: 67000),
                FinishingOption(id: "s4", title: "Дренажная система", deltaRub: 89000)
            ]
        ),
        FinishingCategory(
            title: "Электрика",
            selectionType: .multiple,
            options: [
                FinishingOption(id: "e1", title: "Вводной щиток", deltaRub: 35000),
                FinishingOption(id: "e2", title: "Кабель силовой", deltaRub: 89000),
                FinishingOption(id: "e3", title: "Розетки и выключатели", deltaRub: 45000),
                FinishingOption(id: "e4", title: "Освещение", deltaRub: 67000),
                FinishingOption(id: "e5", title: "Заземление и молниезащита", deltaRub: 55000),
                FinishingOption(id: "e6", title: "Автоматы и УЗО", deltaRub: 28000)
            ]
        ),
        FinishingCategory(
            title: "Вентиляция",
            selectionType: .single,
            options: [
                FinishingOption(id: "v1", title: "Естественная вентиляция", deltaRub: 35000),
                FinishingOption(id: "v2", title: "Приточная вентиляция", deltaRub: 89000),
                FinishingOption(id: "v3", title: "Приточно-вытяжная с рекуперацией", deltaRub: 245000),
                FinishingOption(id: "v4", title: "Воздуховоды", deltaRub: 67000)
            ]
        ),
        FinishingCategory(
            ...
            selectionType: .multiple,
            options: [
                FinishingOption(id: "c1", title: "Сплит-система", deltaRub: 75000),
                FinishingOption(id: "c2", title: "Мульти-сплит система", deltaRub: 185000),
                FinishingOption(id: "c3", title: "Фреонопроводы", deltaRub: 35000)
            ]
        ),
        FinishingCategory(
            ...
            selectionType: .multiple,
            options: [
                FinishingOption(id: "l1", title: "Интернет и телефония", deltaRub: 25000),
                FinishingOption(id: "l2", title: "Телевидение", deltaRub: 15000),
                FinishingOption(id: "l3", title: "Домофон", deltaRub: 18000),
                FinishingOption(id: "l4", title: "Видеонаблюдение", deltaRub: 65000),
                FinishingOption(id: "l5", title: "Охранная сигнализация", deltaRub: 55000)
            ]
        )
    ]
    var currentData: [FinishingCategory] = []
   
    
    var selectedFinishingOptions: Set<String> = []
    var selectedEngineeringOptions: Set<String> = []
    
    let defaultSelectedIds = ["1", "2", "3"]
    func configureCell(for option: FinishingOption, at indexPath: IndexPath) {
        let isSelected = defaultSelectedIds.contains(option.id)
        // настроить ячейку в соответствии с isSelected
    }
    
    
    var sectionItems: [[(name: String, price: String)]] = []
    private var selectedSingleRow: [Int?] = []          // для секций .single
    private var selectedMultipleRows: [Set<Int>] = []   // для секций .multiple
...
//    }()
    ...
    override func viewDidLoad() {
        ...
        selectedSingleRow = Array(repeating: nil, count: currentData.count)
        selectedMultipleRows = Array(repeating: [], count: currentData.count)
    ...
    func saveSelectedOptions() {
        if segmentControl.selectedSegmentIndex == 0 {
            selectedFinishingOptions = getSelectedOptionIds()
        } else {
            selectedEngineeringOptions = getSelectedOptionIds()
        }
    }
    
    func restoreSelectedOptions(for type: DataType) {
        
    }
    func getSelectedOptionIds() -> Set<String> {
        return []
    }
...
extension CalculatingTheHouse: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: CalculatingTheHouseCell.identifier,
        ...
        let option = currentData[indexPath.section].options[indexPath.row]
            ...
            case .single:
                isSelected = (selectedSingleRow[indexPath.section] == indexPath.row)
            case .multiple:
                isSelected = selectedMultipleRows[indexPath.section].contains(indexPath.row)
            }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        switch currentData[indexPath.section].selectionType {
            
        case .single:
            // Находим была ли выбрана ячейка
            let prev = selectedSingleRow[indexPath.section]
            selectedSingleRow[indexPath.section] = indexPath.row
        ...
        case .multiple:
            if selectedMultipleRows[indexPath.section].contains(indexPath.row) {
                selectedMultipleRows[indexPath.section].remove(indexPath.row)
            } else {
                selectedMultipleRows[indexPath.section].insert(indexPath.row)
            }
    ...
    private func calculateTotalSum() -> Double {
        ...
        for (sectionIndex, section) in currentData.enumerated() {
